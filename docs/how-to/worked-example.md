...
CDR (Critical Design Review) is a formal gate confirming the design is complete and fully reviewed. You run the **Gate Sign-Off** workflow, let CI create the `signoff/<feature>/cdr` branch and PR automatically, tick all CDR checklist items, get lead approval, and merge. **On merge, CI creates the `cdr/buck-converter-5v/approved` tag and commits `reviews/library.lock` to `main`.**

---

### Step 3.1 — Run the Gate Sign-Off workflow

Go to **GitHub → Actions → Gate Sign-Off → Run workflow**, enter `buck-converter-5v` as the feature name, and select `cdr`.

> ⏱️ **Wait for CI.** The workflow takes under 30 seconds. It creates `signoff/buck-converter-5v/cdr`, commits `gate-evidence-cdr.md`, and opens the PR automatically.

---

### Step 3.2 — Open the PR link from the workflow summary

Open the completed **Gate Sign-Off** workflow run, then click the PR link in its summary.

**What CI already committed:**
...
> **Do not convert this to a draft.** The CDR checklist is already filled into the PR body when the workflow opens the PR.
...
TRR (Test Readiness Review) confirms the hardware is built, brought up by the designer, and stable enough to hand to the independent IVV team. It does not confirm that IVV has passed — that happens in Stage 6. You run the **Gate Sign-Off** workflow and let CI create the `signoff/<feature>/trr` branch and PR automatically. **On merge, CI creates the `trr/buck-converter-5v/approved` tag.**

---

### Step 5.1 — Run the Gate Sign-Off workflow

Go to **GitHub → Actions → Gate Sign-Off → Run workflow**, enter `buck-converter-5v` as the feature name, and select `trr`.

> ⏱️ Wait for CI. The workflow checks that `cdr/buck-converter-5v/approved` exists, then creates `signoff/buck-converter-5v/trr`, commits `gate-evidence-trr.md`, and opens the PR automatically.

---

### Step 5.2 — Open the PR link from the workflow summary
...
> **Do not convert this to a draft.** The TRR checklist is already filled into the PR body when the workflow opens the PR.
...
This gate formally authorises the manufacturing outputs for production. You run the **Gate Sign-Off** workflow and let CI create the `signoff/<feature>/release` branch and PR automatically. **On merge, CI:**
...
### Step 7.2 — Run the Gate Sign-Off workflow

Go to **GitHub → Actions → Gate Sign-Off → Run workflow**, enter `buck-converter-5v` as the feature name, and select `release`.

> ⏱️ Wait for CI. The workflow creates `signoff/buck-converter-5v/release`, commits `gate-evidence-release.md`, and opens the PR automatically.

---

### Step 7.3 — Open the PR link from the workflow summary
...
> **Do not convert this to a draft.** The Release checklist is already filled into the PR body when the workflow opens the PR.
